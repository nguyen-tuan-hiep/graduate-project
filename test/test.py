...
class LSTMPredictor:
    ...
    def train_model(self, num_epochs=30, learning_rate=0.001):
        ...
        batch_size = 8
        for epoch in range(num_epochs):
            for batch_start in range(0, len(self.X_train_tensors_final), batch_size):
                # Ensure data is on the correct device
                inputs = self.X_train_tensors_final[batch_start:batch_start+batch_size].to(device)
                labels = self.y_train_tensors[batch_start:batch_start+batch_size].to(device)
        ...
        plt.legend()
        plt.grid(True)
        plt.savefig(f'./figure/{self.label}_loss_curve_test.png')
        plt.show()
