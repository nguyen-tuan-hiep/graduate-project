...
# export test_loader to be used in the test.py script
torch.save(test_loader, f'./{data}/test_loader/test_loader_{optim}.pth')
...
for epoch in range(num_epochs):
    ...
    if avg_epoch_val_loss < best_val_loss:
        ...
        # torch.save(best_model, f'./{data}/models/ann_model_{optim}.pth')
...
plt.grid()
plt.savefig(f'./{data}/results/learning_curve_{optim}.png')
plt.savefig(f'./{data}/results/learning_curve_{optim}.pdf')

torch.save(model.state_dict(), f'./{data}/models/model_state_dict_{optim}.pth') 
torch.save(model, f'./{data}/models/model_{optim}.pth')
...
def plot_results(y_true, y_pred):
    ...
    plt.grid()
    plt.savefig(f'./{data}/results/actual_predictions_{optim}.png')
    plt.savefig(f'./{data}/results/actual_predictions_{optim}.pdf')

# model.load_state_dict(torch.load(f'./models/ann_model_{optim}.pth'))
...
with torch.no_grad():  # No need to compute gradients for validation
    ...
    results.to_csv(f'./{data}/results/results_{optim}.csv', index=False)
